...
def warehouse_management_system(account_balance,warehouse,account_history):
    ...
    while True:
        ...
        if user_choice_option == "SALDO": # moduł sprawdzający stan konta, oraz pozwala na jego modyfikacje
            ...
            if user_choice_option_balance == "ODEJMIJ":
                balance_modifier_subtract = float(input("\nWpisz wartość do odjęcia od salda konta: "))
        ...
        if user_choice_option == "ZAKUP":
            print("\n[FUNKCJA: ZAKUP]")
            user_choice_option_buy_product_name = str(input("\nPodaj nazwę produktu: \n")).upper()
            user_choice_option_buy_product_price =float(input("\nPodaj cenę produktu(zł): \n"))
            user_choice_option_buy_product_qty =int(input("\nPodaj ilość sztuk produktu: \n"))

                # sprawdzam czy mnie stać na zakup
            if user_choice_option_buy_product_price * user_choice_option_buy_product_qty <= account_balance["account_balance"]:
        ...
        if user_choice_option == "SPRZEDAŻ":
            print(f"\n[FUNKCJA: SPRZEDAŻ]")
            user_choice_option_sales_item = str(input("\nPodaj nazwę produktu: \n")).upper()
            user_choice_option_sales_qty = int(input("\nPodaj ilość sztuk produktu: \n"))

            # tutaj zdejmuje ze stanu magazynowego produkty które sprzedałem
            if user_choice_option_sales_item in warehouse: # jeśli produkt znajduje się w magazynie
                if warehouse[user_choice_option_sales_item]["ILOŚĆ"] >= user_choice_option_sales_qty: # czy mam wystarczająco sztuk produktu
                        ...
                        print(f"Sprzedano produkt: {user_choice_option_sales_item} | Sztuk: {user_choice_option_sales_qty} | Po cenie jednostkowej: {warehouse[user_choice_option_sales_item]["CENA"]} | Dodano do konta: {(warehouse[user_choice_option_sales_item]["CENA"]*user_choice_option_sales_qty)}")
                        print(f"Po wykonanej sprzedaży saldo wynosi: {round(account_balance["account_balance"],2)}zł")
                        account_history[f"{datetime.now()}"] = f"Sprzedano produkt: {user_choice_option_sales_item} | Sztuk: {user_choice_option_sales_qty} | Po cenie jednostkowej: {warehouse[user_choice_option_sales_item]["CENA"]} | Dodano do konta: {(warehouse[user_choice_option_sales_item]["CENA"]*user_choice_option_sales_qty)}"
                else:
        ...
        if user_choice_option == "PRZEGLĄD":
            print(f"\n[FUNKCJA: PRZEGLĄD]")
            ...
            print(f"Operacje w przedziale czasowym {from_timeline} do {to_timeline}")
